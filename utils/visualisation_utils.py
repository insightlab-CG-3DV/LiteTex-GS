...
def normalise_tensor(tensor):
    ...
    if denominator == 0:
        denominator = tensor.max()
    return (tensor - tensor.min()) / denominator

def show_tensor(input, use_colormap=False):
...
# Takes a N x W x H x C tensor and returns a 
# T x T x C tensor that is the fold tensor
def fold_images(images: torch.Tensor) -> torch.Tensor:
    N = images.shape[0]
...
def show_texture_maps(texture_maps):
    show_tensor(fold_images(texture_maps).clamp(0,1)*255, False)
...
def gaussian_kernel_2d(size, scale, texel_size, cutoff: float = 2, magnitude=None, center_offset=True):
    batch_shape, _ = scale.shape[:-1], scale.shape[-1:]

    offset = 0
    if center_offset:
        offset = 1
    x = torch.arange(0, size[0], device="cuda") - (size[0] - offset) / 2
    y = torch.arange(0, size[1], device="cuda") - (size[1] - offset) / 2
    points = torch.cartesian_prod(x, y)
    points = (points.repeat((*batch_shape, 1, 1)) * (texel_size / scale[..., :2]).view(*batch_shape, 1, 2)).contiguous()
    
    kernels = torch.exp(-0.5*(points.view(-1, 2)**2).sum(dim=-1)).reshape(*batch_shape, size[0], size[1])
    kernels[kernels < np.exp(-0.5*cutoff**2)] = 0
    if magnitude is None:
        return kernels
    return magnitude.view(*batch_shape, 1, 1) * kernels
